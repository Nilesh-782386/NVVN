...
class AIDistributionService {
  ...
  async getCityCoverage(districtOrCity, date = new Date().toISOString().split('T')[0]) {
    try {
      const result = await query(`
        SELECT 
          cca.*,
        ...
        LEFT JOIN ngo_register n ON (n.city = cca.city OR n.district = cca.city)
      ...
      return await this.createCityCoverage(districtOrCity, date);
    } catch (error) {
  ...
  async createCityCoverage(districtOrCity, date) {
    try {
      const statsResult = await query(`
        ...
        FROM donations 
        WHERE (city = ? OR district = ?) AND DATE(created_at) = ?
      `, [districtOrCity, districtOrCity, date]);
  ...
  async getDistributionSuggestions(donationId) {
    try {
      ...
      // Get NGOs in the same district/city with available capacity - Enhanced for all scenarios
      const matchValue = donation.district || donation.city;
      ...
      const ngoResult = await query(`
        ...
        WHERE (
          LOWER(n.district) = LOWER(?) OR 
          LOWER(n.city) = LOWER(?) OR
          (n.district IS NULL AND LOWER(n.city) = LOWER(?)) OR
          (n.city IS NULL AND LOWER(n.district) = LOWER(?))
        )
        ...
        LIMIT 5
      `, [isCritical ? 1 : 0, matchValue, matchValue, matchValue, matchValue, isCritical ? 1 : 0]);
  ...
  async getLoadBalancingRecommendations(districtOrCity) {
    try {
      const result = await query(`
        SELECT 
          n.id,
        ...
        LEFT JOIN ngo_daily_limits ndl ON n.id = ndl.ngo_id AND ndl.date = CURDATE()
        LEFT JOIN ngo_performance np ON n.id = np.ngo_id
        WHERE (LOWER(n.district) = LOWER(?) OR LOWER(n.city) = LOWER(?)) 
          AND n.verification_status = 'approved'
