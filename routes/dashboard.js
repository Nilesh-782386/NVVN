...
router.get("/dashboard/ngo", ensureNGOAuthenticated, async (req, res) => {
  try {
    ...
    res.render("dashboards/ngo-dashboard", {
      ...
      pendingDonations: pendingResult || [],
      confirmedDonations: confirmedResult || [],
      completedDonations: completedResult || [],
      assignedVolunteers: volunteersResult || []
    });
  } catch (err) {
...
router.get("/dashboard/volunteer", ensureVolunteerAuthenticated, async (req, res) => {
  try {
    ...
    // Get volunteer's district for proper matching
    ...
    // Get available donations in volunteer's district/city - Enhanced matching
    const availableResult = await query(`
      ...
      WHERE (
        ...
        AND d.volunteer_id IS NULL AND d.ngo_approval_status = 'approved'
      ORDER BY 
        CASE d.priority 
...
router.get("/api/ngo-locations", async (req, res) => {
  try {
    const result = await query(`
      SELECT id, ngo_name, city, state, latitude, longitude, status
      FROM ngo_register 
      WHERE latitude IS NOT NULL AND longitude IS NOT NULL AND status = 'verified'
    `);
...
router.get("/api/ngo/analytics-data", ensureNGOAuthenticated, async (req, res) => {
  try {
    ...
    const statusData = statusResult && statusResult[0] ? statusResult[0] : {};
    ...
    const volunteerRows = volunteerResult || [];
    const volunteerData = {
    ...
    // Format items data
    const itemsRow = itemsResult && itemsResult[0] ? itemsResult[0] : {};
    const itemsData = {
