...
// Authenticated donation page
router.get("/donate-auth", ensureUserAuthenticated, (req, res) => {
  res.render("donate/donate1", getSafeSession(req));
});
...
router.get("/api/system/status", async (req, res) => {
  try {
    ...
    const metrics = {
      users: usersResult && usersResult[0] ? usersResult[0].count : 0,
      donations: donationsResult && donationsResult[0] ? donationsResult[0].count : 0,
      volunteers: volunteersResult && volunteersResult[0] ? volunteersResult[0].count : 0,
      ngos: ngosResult && ngosResult[0] ? ngosResult[0].count : 0
    };
    ...
    const recentActivity = recentDonations && Array.isArray(recentDonations) ? recentDonations.map(d => 
