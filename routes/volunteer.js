...
router.post(["/volunteer-register", "/volunteer/register"], async (req, res) => {
  ...
  try {
    ...
    if (checkResult && checkResult.length > 0) {
      return res.render("login_register/volunteer_register", {
    ...
    await query(
      `INSERT INTO volunteers (name, fullname, email, phone, password, city, district, vehicle_type, status) 
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'active')`,
      [name, name, email, phone, hash, city, city.toLowerCase(), vehicle_type]
    );

    console.log("✅ Volunteer Registration Successful:", email);
    res.redirect("/volunteer-login?success=registered");
  } catch (err) {
    console.error("❌ Error saving volunteer to database:", err);
