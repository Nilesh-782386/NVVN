...
function initializeNGONetworkMap() {
    if (document.getElementById('ngoNetworkMap')) {
        ...
        ngoNetworkMap = L.map('ngoNetworkMap').setView([21.1458, 79.0882], 10);
        ...
        .then(data => {
            if (data.ngos) {
                data.ngos.forEach(ngo => {
                    // Use default coordinates if latitude/longitude are null
                    const lat = ngo.latitude || (21.1458 + (Math.random() - 0.5) * 0.05);
                    const lng = ngo.longitude || (79.0882 + (Math.random() - 0.5) * 0.05);
        ...
        .then(data => {
            if (data.routes) {
                ...
                data.routes.forEach(route => {
                    // Use default coordinates if latitude/longitude are null
                    const volunteerLat = route.volunteer_lat || (21.1458 + (Math.random() - 0.5) * 0.05);
                    const volunteerLng = route.volunteer_lng || (79.0882 + (Math.random() - 0.5) * 0.05);
                    const ngoLat = route.ngo_lat || (21.1458 + (Math.random() - 0.5) * 0.05);
                    const ngoLng = route.ngo_lng || (79.0882 + (Math.random() - 0.5) * 0.05);
                    
                    // Add volunteer marker
                    const volunteerMarker = L.marker([volunteerLat, volunteerLng], {
                        icon: L.divIcon({
