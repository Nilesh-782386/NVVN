...
// Enhanced accept function with location capture
async function acceptDonationWithDistance(donationId) {
    ...
    let volunteerAddress = null;
    
    if (navigator.geolocation) {
        try {
            console.log('📍 Getting volunteer location...');
            ...
            console.log('📍 Volunteer location captured:', volunteerLocation);
            
            // Try to get address from coordinates (reverse geocoding)
            try {
                const response = await fetch(`https://nominatim.openstreetmap.org/reverse?format=json&lat=${volunteerLocation.latitude}&lon=${volunteerLocation.longitude}`);
                const data = await response.json();
                if (data && data.display_name) {
                    volunteerAddress = data.display_name;
                    console.log('📍 Volunteer address:', volunteerAddress);
                }
            } catch (geocodeError) {
                console.log('⚠️ Could not get address from coordinates:', geocodeError);
            }
            
        } catch (locationError) {
            console.log('⚠️ Could not get volunteer location:', locationError);
        }
    } else {
    ...
    if (volunteerLocation) {
        confirmMessage += `\n\n📍 Your REAL current location will be captured for pickup coordination.`;
        confirmMessage += `\n🎯 This will be used as the pickup starting point.`;
    } else {
        confirmMessage += `\n\n📍 NGO's district location will be used for pickup coordination.`;
        confirmMessage += `\n⚠️ Your real location could not be captured.`;
    }
        ...
        try {
            const response = await fetch(`/accept-donation/${donationId}`, {
                ...
                body: JSON.stringify({
                    ...
                    volunteerLatitude: volunteerLocation ? volunteerLocation.latitude : null,
                    volunteerLongitude: volunteerLocation ? volunteerLocation.longitude : null,
            ...
            if (result.success) {
                ...
                let message = '✅ Donation accepted successfully!';
                if (distance) {
                    message += `\n📏 Distance: ${distance} km`;
                }
                if (result.pickupLocation && result.pickupLocation.address) {
                    if (volunteerLocation) {
                        message += `\n📍 Your REAL location captured: ${result.pickupLocation.address}`;
                    } else {
                        message += `\n📍 Using NGO district: ${result.pickupLocation.address}`;
                    }
                }
